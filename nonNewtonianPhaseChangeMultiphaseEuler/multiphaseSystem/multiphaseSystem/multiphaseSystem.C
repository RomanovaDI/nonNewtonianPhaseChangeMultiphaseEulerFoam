...
/*
    ...
    for (phaseModel& phase1 : phases_)
    {
        ...
        for (phaseModel& phase2 : phases_)
        {
        	surfaceScalarField& alphaPhiCorr2 = alphaPhiCorrs[phaseii];
        	//volScalarField& alpha2 = phase2;
    		...
    		{
        	    if (
					(&phase1 == &mtmIter()->phase1()) 
				  & (&phase2 == &mtmIter()->phase2()))
        	    {
        			alphaPhiCorr -= fvc::interpolate(
						massTransferCoeff(phase1) * alpha1);
        			alphaPhiCorr2 += fvc::interpolate(
						massTransferCoeff(phase1) * alpha1);
        	    }
			}
...
*/
    ...
    for (phaseModel& phase : phases_)
    {
		...
		// Adding Sp and Su terms t o MULES explicit solve
		const volScalarField Sp(mtmSp(phase, massTransferCoeffs()));
		const volScalarField Su(mtmSu(phase, massTransferCoeffs()));
        ...
        Info<< phase.name() << " Sp volume fraction, min, max = "
            << Sp.weightedAverage(mesh_.V()).value()
            << ' ' << min(Sp).value()
            << ' ' << max(Sp).value()
            << endl;

        Info<< phase.name() << " Su volume fraction, min, max = "
...
Foam::autoPtr<Foam::multiphaseSystem::massTransferCoeffFields>
Foam::multiphaseSystem::massTransferCoeffs() const
{
...
Foam::tmp<Foam::volScalarField>Foam::multiphaseSystem::mtmSp
(
	const phaseModel& phase ,
...
Foam::tmp<Foam::volScalarField>Foam::multiphaseSystem::mtmSu
(
	const phaseModel& phase ,
	const massTransferCoeffFields& massTransferCoeffField
) const
...
Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::massTransferCoeff
(
    const phaseModel& phase,
    const massTransferCoeffFields& massTransferCoeffs
) const
{
    tmp<volScalarField> tmassTransferCoeff
    (
        new volScalarField
        (
            IOobject
    ...
    massTransferModelTable::const_iterator mtmIter = massTransferModels_.begin();
    massTransferCoeffFields::const_iterator mtcIter = massTransferCoeffs.begin();
    for
    (
        ;
        mtmIter.good() && mtcIter.good();
        ++mtmIter, ++mtcIter
    )
    {
        if
        ...
        {
            tmassTransferCoeff.ref() += *mtcIter();
        }
    }

    return tmassTransferCoeff;
}
...
void Foam::multiphaseSystem::calcStrainRateTensors2Inv()
{
    for (phaseModel& phase : phases_)
    {
